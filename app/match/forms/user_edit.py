...
class EditForm(forms.Form):
    ...
    error_message = ''
    username = forms.CharField(label='LOGIN_ID', max_length=20, widget=forms.TextInput(attrs={'placeholder':'名前を入力して下さい', 'class':'form-control'}))
    password = forms.CharField(label='PASSWORD', max_length=20, widget=forms.PasswordInput(attrs={'placeholder':'パスワードを入力して下さい', 'class':'form-control', 'autocomplete':'off'}))
    SEX = [('man','男'),('woman','女')]
    sex = forms.ChoiceField(choices=SEX, widget=forms.RadioSelect())
    self_introduction = forms.CharField(required=False, label='SELF_INTRODUCTION', max_length=200, widget=forms.Textarea(attrs={'rows':10, 'class':'form-control'}))
    info = UserInfo()
    is_save = False
